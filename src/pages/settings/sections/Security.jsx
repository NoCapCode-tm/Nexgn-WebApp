...
export default function Security({ user, onUserUpdated }) {
  ...
  // 1. Restore enable2FA to local state so the toggle moves instantly when clicked
  ...
  // 2. Sync the backend user data into the local state
  useEffect(() => {
    // Check your browser console! This will tell you exactly what field names your backend uses.
    console.log("Security Component received user:", user);

    if (user) {
      // Added `user.twoFAsecret` based on hints from your backend files
      const isActive = Boolean(
        user.isTwoFactorEnabled ?? 
        user.twoFAEnabled ?? 
        user.is2FAEnabled ?? 
        user.twoFAsecret // If a secret exists, 2FA is likely on
      );
      
      setSecurityData((prev) => ({ ...prev, enable2FA: isActive }));
    }
  }, [user]);
  ...
  const handle2FAToggle = async (e) => {
    ...
    // Optimistically update the UI so the toggle moves instantly
    setSecurityData((prev) => ({ ...prev, enable2FA: enabled }));
  ...
  return (
    <>
      <div className="admin-settings-card admin-settings-card--security">
        ...
        <form className="admin-settings-form" onSubmit={handleUpdate} id="admin-settings-security-form">
          ...
          <div className="admin-settings-form__footer">
            <button type="submit" className="admin-settings-form__submit" id="admin-settings-update-security-btn">
              Update Password
            </button>
          </div>
        </form>
      ...
      {show2FAOverlay && (
        <div className="admin-2fa-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) close2FAOverlay(); }}>
          <div className="admin-2fa-modal" onMouseDown={(e) => e.stopPropagation()}>
            <button type="button" className="admin-2fa-close-btn" onClick={close2FAOverlay} aria-label="Close" title="Close">
            ...
            {!showOTPInput ? (
              <>
                ...
                <div className="admin-2fa-or"><span>OR</span></div>
                ...
                <div className="admin-2fa-input-wrapper">
                  ...
                  <div className="admin-2fa-corner bl" />
                  <div className="admin-2fa-corner br" />
                  <input type="text" className="admin-2fa-manual-input" value={twoFASecret} readOnly />
                </div>
                <button type="button" className="admin-2fa-btn-primary" onClick={() => setShowOTPInput(true)}>
                  Verify
                </button>
              </>
            ) : (
              <>
                ...
                <div className="admin-2fa-otp-wrapper" onClick={() => otpInputRef.current?.focus()}>
                  <div className="admin-2fa-corner tl" />
                  <div className="admin-2fa-corner tr" />
                  <div className="admin-otp-boxes">
                    {Array.from({ length: 6 }).map((_, i) => (
                      <div key={i} className={`admin-otp-box ${otp[i] ? "filled" : ""}`}>
                        {otp[i] || ""}
                      </div>
                    ))}
                  </div>
                ...
                <div className="admin-2fa-actions">
                  <button type="button" className="admin-2fa-btn-primary" disabled={otp.length !== 6 || twoFALoading} onClick={verify2FA}>
                    {twoFALoading ? "Verifying..." : "Verify"}
                  </button>
                  <button type="button" className="admin-2fa-btn-outline" onClick={() => setShowOTPInput(false)}>
                    Back
                  </button>
                </div>
              </>
      ...
      {loading && (
        <LoadingScreen state="connecting" size={64} theme="dark" message="Applying your master plan" />
      )}
    </>
  );
}